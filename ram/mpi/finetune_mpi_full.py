...
model = finetune(model_ram, y, physics, early_stop=False, transform=None, max_iter=epoch_nb, noise_loss='noiseless', lr=1e-5, batch_size=batch_size)
...
# Save trained parameters LETS SEE
ckpt_dir = os.path.join(PROJECT_ROOT, "ram", "mpi", "checkpoints")
os.makedirs(ckpt_dir, exist_ok=True)
ckpt_path = os.path.join(ckpt_dir, f"ram_finetuned_x{scale_factor}.pth")

torch.save(
    {
        "state_dict": model_ram.state_dict(),
        "scale_factor": scale_factor,
    },
    ckpt_path,
)
...
# Create a fresh RAM model and load trained parameters for evaluation
eval_model = RAM(device=device).to(device)
checkpoint = torch.load(ckpt_path, map_location=device)
eval_model.load_state_dict(checkpoint["state_dict"], strict=True)
eval_model.eval()

# Evaluation inference with the reloaded model
with torch.no_grad():
    x_eval = eval_model(y, physics=physics)

print("Reloaded model inference done.")
print(f"Max |x_hat - x_eval| = {(x_hat - x_eval).abs().max().item():.6e}")
